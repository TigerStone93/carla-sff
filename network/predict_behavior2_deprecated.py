...
class PredictBehavior:
    def __init__(self, name="0", reuse=False, learner_lr=0.0001, learner_reg=0.001, learner_prob_reg=0.0001):
        ...
        with tf.variable_scope("PredictBehavior_" + name, reuse=reuse):
            ...
            self.layer_input_target = tf.placeholder(tf.float32, [None, 4])
            ...
            self.target_log_prob = tf.reduce_sum(tf.reshape(target_log_prob, [-1, 4, 4]), axis=2)
            ...
            likelihood_loss = tf.reshape((self.reparameterized - target_output) ** 2, [-1, 4, 4])
            self.dist_likelihood_loss =  tf.reduce_mean(tf.reduce_min(likelihood_loss, axis=2) + tf.reduce_mean(likelihood_loss, axis=2) * 0.001)
            self.dist_regularization_loss = tf.reduce_mean(self.mu ** 2 + tf.exp(self.logsig_clipped) ** 2 - self.logsig_clipped ** 2 - 1.)

            self.raw_stoc_output = tf.reshape(self.reparameterized, [-1, 4, 4])
            self.stoc_output = self.raw_stoc_output * tf.reshape(self.prob, [-1, 4, 1])
